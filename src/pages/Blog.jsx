...
const Blog = () => {
  ...
  const { title, published_at, comments_count, public_reactions_count } = data;
  const handaleBookMark = blog => {
    saveBlog(blog);
  };

  if (navigation.state === 'loading') return <Loder />;
      ...
      <article className="space-y-8 ">
        <div className="space-y-6">
          ...
          <div className="flex flex-col items-start justify-between w-full md:flex-row md:items-center ">
            <p className="text-sm">
              5 min red {new Date(published_at).toLocaleDateString()}
            </p>
          ...
          <div className="flex items-center -mx-4 overflow-x-auto overflow-y-hidden sm:justify-start flex-nowrap ">
            ...
            {/* Bookmark Buttob */}
            <div
              onClick={() => handaleBookMark(data)}
              className="bg-base-300 p-3 ml-5 rounded-full hover:bg-opacity-30 bg-opacity-20 cursor-pointer hover:scale-105 overflow-hidden"
            >
              <MdBookmarkAdd
                size={20}
